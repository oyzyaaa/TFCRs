...
int main(int argc, char** argv) {
    ...
    while (!feof(fp))
    {
		...
		if((strcmp(prechr,chrname)!=0)||((tempPos-prePos)>2*Width))
		{
			...
			if(cli==1)
			{
				long start=cluster[0]-150;
				long end=cluster[0]+150;
				fprintf(fout,"%s\t%d\t%d\t1\t1\n",prechr,start,end);
			}
			else
			{
				double *scores;
				...
				//for(ii=cluster[0];ii<=cluster[cli-1];ii++)
				for(jj=1;jj<cluster[cli-1]-cluster[0];jj++)
				{
					if(((scores[jj]>=1)&&((scores[jj]-scores[jj-1])>0)&&((scores[jj]-scores[jj+1])>=0)))
					{
						//you are max
						int kk=0;
						for(kk=643;kk>=0;kk--)
						...
						long start=jj-kk-150 + cluster[0];
						long end =jj+kk+150 + cluster[0];
							
						double tvalue=0;
						for(kk=-643;kk<643;kk++)
						{
